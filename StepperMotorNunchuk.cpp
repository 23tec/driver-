...
void runStepper(unsigned long usDelay) {
    ...
    const unsigned long highTime = constrain(usDelay / 50, 2, 10);
    const unsigned long lowTime = usDelay;
    const unsigned long currentTime = micros();

    if (usDelay == ULONG_MAX) 
        return;
...
void loop() {
    ...
    if (!endstopTriggered) {
        // In automatico, leggere il Nunchuk può causare microvariazioni
        // udibili nello stepper.
        if (!automatic) {
