...
/// @brief Hardware
const float encoderPPR = 1000.0; // Pulse per revolution
const float stepperStepsPerRev = 200.0; // 1.8° per step
const float microsteps = 2.0; // DM542 impostato a 1/2 step
const float microstepsPerRev = stepperStepsPerRev * microsteps; // 400
const float leadscrewPitchMM = 2.5; // Passo madrevite asse Z
const float distanceToTravelMM = 10.0; // 1 cm
const float stepsFor1mm = microstepsPerRev / leadscrewPitchMM; // 160
///
...
int stickAngle = 0;
volatile bool stepRequested = false;
volatile bool stepPending = false;
volatile bool pulseHigh = false;
volatile long absoluteEncoderSteps = 0;
volatile long stepperSteps = 0;
...
void runStepper(unsigned long pulseWidth) {
    ...
    // Uso la ISR direttamente, qui stepPending è irrilevante 
    stepRequested = true;
    delayMicroseconds(currentDelay);
...
// Jog wheel 1:1 with stepper motor
void encoderISR() {
    if (stepPending) // Anti-Jitter
        return;
    ...
    absoluteEncoderSteps += (dir == DIR_FORWARD) ? 1 : -1;
...
ISR(TIMER1_COMPA_vect) {
    if (stepRequested) {
        ...
        if (!pulseHigh) {
            ...
            PORTB &= ~(1 << PB2);
            pulseHigh = false;
            stepperSteps += ((stepperDir == DIR_FORWARD) ? 1 : -1);
            stepRequested = false;
            stepPending = false;
        }
    }
...
void handleEndstops() {
    static long stepsToReleaseEndstop = 0;
    static long stoppedAtPosition = 0;
    //static float mmMoved = 0.0;
    
    
    if (endstopTriggered) {
        noInterrupts();
        long currentSteps = absoluteEncoderSteps;
        interrupts();
   
        stepsToReleaseEndstop = currentSteps - stoppedAtPosition;
        //mmMoved = (float) stepsToReleaseEndstop / stepsFor1mm;
        
        if ((allowedDirection == DIR_BACKWARD && stepsToReleaseEndstop <= -stepsFor1mm * distanceToTravelMM)
          || (allowedDirection == DIR_FORWARD && stepsToReleaseEndstop >= stepsFor1mm * distanceToTravelMM)) {
    ...
    if (endstop0 || endstop1) {
        ...
        noInterrupts();
        stoppedAtPosition = absoluteEncoderSteps;
        interrupts();
        
        stepsToReleaseEndstop = 0;
        //mmMoved = 0.0;
    }
...
unsigned long motionProfile2(const unsigned long startMotionTime, 
                             const unsigned long minAccelTime, 
                             const unsigned long maxAccelTime) { 
    // Con avanzamento lento non serve una accelerazione
    if (stepperPulseDelay >= 1300)
        return stepperPulseDelay;

    unsigned long elapsed = millis() - startMotionTime;
    if (elapsed < maxAccelTime) {
        float ratio = float(elapsed) / maxAccelTime;
        float eased = (1.0 - cos(ratio * PI)) / 2.0; // S-curve

        return minAccelTime - eased * (minAccelTime- stepperPulseDelay);
    } 
...
void loop() {
    ...
    if (!endstopTriggered) {
        if (!autofeedActived) {        
            ...
            // Autofeed attivo
            runStepper(motionProfile2(motionStartTime, 800, 2000));
        }
    }


    // Jog wheel non in uso spegni lo stepper
